...
EmberAfStatus emberAfGreenPowerClusterServerCommandParse(EmberAfClusterCommand *cmd)
{
  ...
  if (!cmd->mfgSpecific) {
    ...
    case ZCL_GP_PAIRING_SEARCH_COMMAND_ID:
      {
        uint16_t payloadOffset = cmd->payloadStartIndex;
        uint16_t options;  // Ver.: since gp-1.0-09-5499-24
        uint32_t gpdSrcId;  // Ver.: since gp-1.0-09-5499-24
        ...
        wasHandled = emberAfGreenPowerClusterGpPairingSearchCallback(options,
                                                                     gpdSrcId,
                                                                     gpdIeee,
                                                                     endpoint);
        break;
    ...
    case ZCL_GP_PROXY_TABLE_RESPONSE_COMMAND_ID:
      {
        uint16_t payloadOffset = cmd->payloadStartIndex;
        uint8_t status;  // Ver.: always
        uint8_t totalNumberOfNonEmptyProxyTableEntries;  // Ver.: always
        uint8_t startIndex;  // Ver.: always
        uint8_t entriesCount;  // Ver.: always
        uint8_t* proxyTableEntries;  // Ver.: always
        // Command is fixed length: 4
        if (cmd->bufLen < payloadOffset + 4u) { return EMBER_ZCL_STATUS_MALFORMED_COMMAND; }
        status = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 1u;
        totalNumberOfNonEmptyProxyTableEntries = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 1u;
        startIndex = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 1u;
        entriesCount = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
        payloadOffset += 1u;
        proxyTableEntries = cmd->buffer + payloadOffset;
        wasHandled = emberAfGreenPowerClusterGpProxyTableResponseCallback(status,
